...
export class ReservationValidators {
  static async validateReservationParams(
    jsonObj,
    password,
    hotelReservationId,
    reservationCostLOC,
    ...
    if (
      hotelReservationId > bytesParamsLength ||
      hotelId > bytesParamsLength ||
      roomId > bytesParamsLength
    ) {
      throw new Error(ERROR.INVALID_ID_PARAM);
    }

    if (
      daysBeforeStartForRefund.length > maxRefundPeriods ||
    ...
    for (let i = 0; i < daysBeforeStartForRefund.length - 1; i++) {
      if (
        daysBeforeStartForRefund[i] * 1 <
        daysBeforeStartForRefund[i + 1] * 1
      ) {
  ...
  static async validateSimpleReservationCustomWithdrawerParams(
    jsonObj,
    password,
    hotelReservationId,
    reservationCostLOC,
    ...
    if (
      ((Date.now() / secondsInMilliSeconds) | 0) >
      withdrawDateInSecondsFormatted
    ) {
    ...
    await this.validateSimpleReservationMultipleWithdrawersDontExist(
      hotelReservationId
    );
  ...
  static async validateSimpleReservationSingleWithdrawerParams(
    jsonObj,
    password,
    reservationCostLOC,
    withdrawDateInDays
  ...
  static async validateBookingExists(hotelReservationId) {
    ...
    if (
      bookingContractAddress === "0x0000000000000000000000000000000000000000"
    ) {
  ...
  static validateReservationDates(
    reservationStartDate,
    reservationEndDate,
    daysBeforeStartForRefund
  ) {
    const nowUnixFormatted = formatTimestamp(
      (new Date().getTime() / secondsInMilliSeconds) | 0
    );
    let dayInSeconds = 60 * 60 * 24;
    let yearsPeriodInSeconds =
      (dayInSeconds * yearInDays + leapYearDay * 2) * yearsForTimeValidation;
    if (
      reservationStartDate < nowUnixFormatted ||
      reservationStartDate > nowUnixFormatted + yearsPeriodInSeconds ||
      reservationStartDate.toString().length !== timestampInSecondsLength
    ) {
    ...
    if (
      reservationEndDate > nowUnixFormatted + yearsPeriodInSeconds ||
      reservationEndDate.toString().length !== timestampInSecondsLength
    ) {
    ...
    for (let i = 0; i < daysBeforeStartForRefund.length; i++)
      if (
        nowUnixFormatted + daysBeforeStartForRefund[i] * dayInSeconds >
        reservationStartDate
      ) {
  ...
  static validateWithdrawDate(withdrawDate) {
    ...
    if (
      withdrawDate > nowUnixFormatted + yearsPeriodInSeconds ||
      withdrawDate.toString().length !== timestampInSecondsLength
    ) {
      throw new Error(ERROR.INVALID_WITHDRAW_DATE);
    }
  }
  ...
  static validateWithdrawDateInDays(withdrawDate) {
    const nowDaysFormatted = formatTimestampToDays(
      (new Date().getTime() / secondsInMilliSeconds) | 0
    );
    let dayInSeconds = 60 * 60 * 24;
    let yearsPeriodInSeconds =
      (dayInSeconds * yearInDays + leapYearDay * 2) * yearsForTimeValidation;
    if (
      withdrawDate > nowDaysFormatted + yearsPeriodInSeconds ||
      withdrawDate.toString().length !== timestampInDaysLength
    ) {
      throw new Error(ERROR.INVALID_WITHDRAW_DATE);
    }
  }

  static async validateSimpleReservationMultipleWithdrawersDontExist(
    hotelReservationId
  ) {
    let recipientAddress = await SimpleReservationMultipleWithdrawersContract.reservations(
      hotelReservationId
    );
  ...
  static validateCancellation(
    ...
    for (let i = 0; i < daysBeforeStartForRefund.length; i++) {
      let daysBeforeStartForRefundAddedToNow =
        (addDaysToNow(+daysBeforeStartForRefund[i]).getTime() /
          secondsInMilliSeconds) |
        0;
      if (
        refundPercentages[i] <= 0 ||
  ...
  static validateDispute(
    ...
    if (
      customerAddress !== senderAddress ||
      currentTimestamp < reservationStartDate ||
      currentTimestamp > reservationEndDate
    ) {
  ...
  static async validateWithdrawFunds(
    jsonObj,
    password,
    ...
    if (
      ...
      !password ||
      !reservationIdsArrayBytes ||
      reservationIdsArrayBytes.length < 1
    ) {
      throw new Error(ERROR.INVALID_PARAMS);
    }
    const currentTimestamp = (Date.now() / secondsInMilliSeconds) | 0;
    ...
    for (let i = 0; i < reservationIdsArrayBytes.length; i++) {
      ...
      let reservationMapping = await SimpleReservationMultipleWithdrawersContract.reservations(
        reservationIdsArrayBytes[i]
      );
    ...
    if (
      reservationIdsArrayBytes.length > maxAllowedCyclesForWithdraw.toString()
    ) {
