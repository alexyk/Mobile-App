...
export function getNodeProvider() {
  ...
  if (currentNetwork === "local") {
    return new providers.JsonRpcProvider(
      Config["ETHERS_HTTP_PROVIDER_LOCAL"],
      providers.networks.unspecified
    );
  }
  return new providers.InfuraProvider(
    providers.networks[currentNetwork],
...
export let LOCExchangeContract = new ethers.Contract(
  Config["LOCExchange"],
...
export let HotelReservationFactoryContract = new ethers.Contract(
  Config["HotelReservationFactoryProxy"],
...
export let SimpleReservationMultipleWithdrawersContract = new ethers.Contract(
  Config["SimpleReservationMultipleWithdrawers"],
  SimpleReservationMultipleWithdrawersJSON.abi,
  nodeProvider
);
...
export let SimpleReservationSingleWithdrawerContract = new ethers.Contract(
  Config["SimpleReservationSingleWithdrawer"],
  SimpleReservationSingleWithdrawerJSON.abi,
  nodeProvider
);
