...
import { ExchangeValidators } from "../validators/exchangeValidators";
...
export async function getGasPrice() {
  try {
    ...
    return ethers.utils.parseUnits(
      (response.data.fast / 10).toString(10),
      "gwei"
    );
  } catch (e) {
...
export async function exchangeLocForEth(
  walletAddress,
  walletPrivateKey,
  amount
) {
...
export async function fundTransactionAmountIfNeeded(
  walletAddress,
  walletPrivateKey,
  actionGas = 0
) {
  ...
  if (gasAmountNeeded.gt(accountBalance) && nonceNumber < nonceMaxNumber) {
    ...
    result.fundInitialGas = await axios.post(
      "http://localhost:8080" + JAVA_REST_API_SEND_FUNDS,
      {
  ...
  const minAllowedGasAmountFirst = gasAmountAction.add(gasAmountNeeded);
  const minAllowedGasAmount = minAllowedGasAmountFirst.mul(
    gasConfig.MIN_TIMES_GAS_AMOUNT
  );
