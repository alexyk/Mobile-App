import { LOCTokenContractWithWallet } from "../config/contracts-config.js";
import { getNonceNumber } from "./ethFuncs";

const gasConfig = require("./../config/gas-config.json");
const ERROR = require("./../config/errors.json");
const { failedSatusCode } = require("../config/constants.json");
...
export async function approveContract(
  ...
  const allowance = await locContract.allowance(
    wallet.address,
    contractAddressToApprove
  );
  ...
  const approve = await locContract.approve(
    contractAddressToApprove,
    amount,
    overrideOptions
  );
