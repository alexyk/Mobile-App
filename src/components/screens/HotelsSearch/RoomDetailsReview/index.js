...
class RoomDetailsReview extends Component {
        ...
        if (res.success) {
          ...
          res.body
            .then(data => {
              ...
              requester.quoteBooking(quoteBookingCandidate).then(res => {
  ...
  requestSafechargeMode = () => {
    requester.getConfigVarByName(SAFECHARGE_VAR).then(res => {
  ...
  requestLockOnQuoteId(paymentMethod) {
    ...
    if (quoteId) {
      ...
      //console.log(body);
      return requester
        .markQuoteIdAsLocked(quoteId, body)
        .then(res => res.body)
        .then(res => {
  ...
  payWithLocSingleWithdrawer = () => {
    ...
    this.refs.toast.show(
      "We are working on your transaction this might take some time.",
      1000,
      () => {
        ...
        this.requestLockOnQuoteId("privateWallet").then(() => {
          ...
          if (
            !(
              ...
              locAmounts[DEFAULT_QUOTE_LOC_ID].locAmount
            )
          ) {
            this.setState({ isLoading: false }, () => {
              this.refs.toast.show(
                "Sorry, Cannot get Loc Amount, Please try again later.",
                1000
              );
            });
            return;
          ...
          const endDate = moment
            .utc(booking.arrivalDate, "YYYY-MM-DD")
                ...
                .then(transaction => {
                  ...
                  this.setState({ isLoading: false }, () => {
                    setTimeout(() => {
                      ...
                      requester
                        .confirmBooking(bookingConfirmObj)
                        .then(() => {
                          ...
                          this.refs.toast.show(
                            "LOC Payment has been initiated. We will send you a confirmation message once it has been processed by the Blockchain.",
                            2000,
                            () => {
                        ...
                        .catch(error => {
                          ...
                          this.restartQuote();
                          this.refs.toast.show(
                            "Something with your transaction went wrong...",
                            2000
                          );
                        });
                    }, 1000);
                    ...
                    setTimeout(() => {
                      if (error.hasOwnProperty("message")) {
                        if (error.message === "nonce too low") {
                          this.refs.toast.show(
                            "You have a pending transaction. Please try again later.",
                            2000
                          );
                        } else {
  ...
  onConfirm = () => {
    if (hotelSearchIsNative.step4Payment) {
      ...
      const {
        id: hotelBookingId,
        hotelId
      } = this.state.data.booking.hotelBooking[0];
    ...
    requester
      ...
      .then(data => {
        if (data.userHasPendingBooking) {
          this.ref.toast.show(
            "You already have one active booking with status PENDING in your dashboard. This booking is still being processed and is pending confirmation from the hotel.",
            5000
          );
        ...
        <View style={styles.modalView}>
          <View style={styles.popup}>
            <View style={styles.labelCloseView}>
              <Text style={styles.walletPasswordLabel}>
                Enter your wallet password
              </Text>
              <View style={styles.closeButtonView}>
                <TouchableOpacity onPress={this.closeConfirmModal}>
                  <Image
                    style={styles.closeButtonSvg}
            ...
            <TextInput
              ...
              onChangeText={walletPassword =>
                this.setState({ password: walletPassword })
              }
            ...
            />
            <TouchableOpacity onPress={this.payWithLocSingleWithdrawer}>
        ...
        <View style={styles.modalView}>
          <View style={styles.popup}>
            ...
            <View style={{ flexDirection: "column", marginTop: 10 }}>
              <Text style={{ fontFamily: "FuturaStd-Light" }}>
                Cancellation fee before {this.state.cancelationDate}
              </Text>
              <Text style={{ fontFamily: "FuturaStd-Light" }}>
                {params.currency} 0.00 (0.0000 LOC)
              </Text>
            </View>
            <View style={{ flexDirection: "column", marginTop: 10 }}>
              <Text style={{ fontFamily: "FuturaStd-Light" }}>
                Cancel on {this.state.cancelationDate}
              </Text>
              <Text style={{ fontFamily: "FuturaStd-Light" }}>
                {params.currency} {this.state.cancellationPrice} - (
                {this.state.cancellationPrice} LOC)
              </Text>
            </View>
            <TouchableOpacity
            ...
            <View style={styles.hotelInfoContainer}>
              ...
              <View style={styles.hotelInfoView}>
                ...
                <Text style={styles.hotelPlace}>
                  {params.hotelDetails.additionalInfo.mainAddress}
                </Text>
              </View>
            </View>
          </View>
          ...
          <View style={styles.listItem}>
            ...
            <View style={styles.listItemRhsWrapper}>
              <Text style={styles.rhs}>
                {this.state.arrivalDate} - {this.state.leavingDate}
              </Text>
            </View>
          </View>
          <View style={styles.listItem}>
