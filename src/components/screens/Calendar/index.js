...
class Calendar extends Component {
  ...
  constructor(props) {
    ...
    const { today, checkInMoment, checkOutMoment } = props.datesAndGuestsData;
    ...
    this.initialState = {
      ...
      isLoading: true,
      weekDays: [7, 1, 2, 3, 4, 5, 6].map(item => (
        <Text style={[styles.weekText, subFontColor]} key={item}>
          {i18n(this.year, item, "w")}
        </Text>
      ))
    };
  ...
  onChoose(day) {
    ...
    this.prevCheckOutMoment = prevCheckOutMoment
      ? prevCheckOutMoment
  ...
  render() {
    ...
    const { mainColor, subColor, borderColor, primaryColor } = color;
