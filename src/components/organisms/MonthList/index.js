...
export default class MonthList extends PureComponent {
  ...
  componentDidCatch(error, errorInfo) {
    processError(`[MonthList] Error in component: ${error.message}`, {
      error,
      errorInfo
    });
  }
  ...
  _getItemHeight(data, index, heightCorrection, rowCorrection) {
    ...
    const rowsCount = item && item.days ? Math.round(item.days.length / 7) : 0;
  ...
  _getItemLayout(data, currentIndex) {
    ...
    const currentItemHeight = this._getItemHeight(
      data,
      currentIndex,
      heightCorrection,
      rowCorrection
  ...
  _scrollToSelectedMonth() {
    ...
    setTimeout(() => {
      ...
      if (this.list && index > -1) {
        ...
        processError(
          `[MonthList] Trying to scroll to index ${index} failed - this.list or item is not defined. Try again: ${tryAgain}`,
          { hasList: this.list != null, tryAgain }
        );
      }
    }, 200);
  }
  ...
  _renderDay(item, index, markedData) {
    ...
    const rendered = (
      <Day {...dayProps} onChoose={this.props.onChoose} key={id} />
    );
  ...
  _renderDaysRow(days, index, marked, id) {
    const result = (
      <View style={monthListStyles.dayRow} key={id}>
        {days.map((item, index) => this._renderDay(item, index, marked))}
      </View>
    );
  ...
  _renderMonth({ item, index }) {
    const { markedMonths, color } = this.props;
