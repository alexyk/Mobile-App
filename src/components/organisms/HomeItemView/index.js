...
class HomeItemView extends Component {
  ...
  render() {
    const { item, currencySign, currency } = this.props;
    ...
    const price =
      item.prices && currency === item.currency_code
        ? parseFloat(item.defaultDailyPrice, 10).toFixed()
        : parseFloat(item.prices[currency], 10).toFixed(2);
    //let price = item.defaultDailyPrice / this.props.daysDifference;
    const priceInRoomsCurrency =
      item.prices && item.prices[RoomsXMLCurrency.get()];
