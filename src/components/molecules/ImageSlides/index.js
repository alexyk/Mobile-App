...
export default class ImageSlides extends Component {
  ...
  _renderSlideNumber(current, total) {
    return (
      <View style={{position:'absolute', bottom: 40, right: 30, backgroundColor: '#abc9', padding: 5, borderRadius: 10}}>
  ...
  _renderImage({ itemIndex, currentIndex, item, animatedValue }, width, height) {
    return (
      <Image
        style={{width, height}}
        source={item}
  ...
  render() {
    ...
    const logoHeight = ( height != null ? height : (windowHeight * 0.3 + getSafeTopOffset()) );
