...
class SmallPropertyTile extends Component {
  ...
  async componentDidMount() {
    const property = await requester
      .getListing(this.props.listing.id)
      .then(res => res.body);
  ...
  render() {
    ...
    if (listingsType === "homes") {
      ...
      listingPrice = 0.0;
      photos = hotelPhotos.map(x => ({
        thumbnail: `http://roomsxml.com${x.externalThumbnailUrl}`
      }));
      rating = star;
    }
    return (
      <View style={styles.container}>
        ...
        {property ? (
          <View>
            ...
            {currencyConversion ? (
              <Text style={styles.costText}>{`${currencySign}${(
                property.defaultDailyPrice * currencyConversion
              ).toFixed(2)} (LOC ${(
                property.defaultDailyPrice *
                currencyConversion *
                locRate
              ).toFixed(2)}) per night`}</Text>
            ) : null}
          </View>
        ) : null}
      </View>
