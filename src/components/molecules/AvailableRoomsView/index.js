...
class AvailableRoomsView extends Component {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    const { isSearchDone } = this.props;
    const { rooms } = nextState;
    
    if (nextProps.isSearchDone != isSearchDone) {
      if (!isSearchDone && (rooms == null || rooms.length == 0)) {
        this.checkRooms();
      }
    }
    return true;
  ...
  checkRooms() {
    let request = this.props.search.replace(/\?/gi, "").split("&");
    serverRequest(this, requester.getHotelRooms, [this.props.id, request],
      data => {
        let roomsResults = [];
        const rooms = data;
        if (rooms) {
          ...
          for (let key in usedRoomsByTypeAndMeal) {
            roomsResults.push(
              usedRoomsByTypeAndMeal[key].sort((x, y) =>
                x.totalPrice > y.totalPrice ? 1 : -1
              )
            );
          }
          roomsResults = roomsResults.sort((x, y) =>
            this.getTotalPrice(x[0].roomsResults) > this.getTotalPrice(y[0].roomsResults)
              ? 1
              : -1
          );
        }
          ...
          >
            ...
            <View style={{ flexDirection: "row" }}>
              <Text style={styles.price}>
                ...
                {currencySign}
                {rowData.roomsResults[0].price === undefined ? "" : price}
              </Text>
  ...
  _renderLoader() {
    return (
      <LTLoader isLockTripIcon opacity={"00"} style={{ marginVertical: 25 }} />
    );
  }
  ...
  _renderRooms() {
    ...
    if (rooms == null || rooms.length == 0) {
      const message = (
        isSearchDone
        ? lang.TEXT.NO_ROOMS
        : lang.TEXT.NO_ROOMS_SEARCH_IN_PROGRESS
      )
      return (
        <View>
          <Text
  ...
  render() {
    return (
      <View style={styles.container}>
        ...
        {this.state.isLoading ? this._renderLoader() : this._renderRooms()}
      </View>
    );
  }
...
let mapStateToProps = state => {
  return {
    isSearchDone: state.hotels.isSearchDone,
