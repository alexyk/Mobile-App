...
class ConfirmBottomBar extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (
      ...
      nextProps.params.bookingId !== "" &&
      this.props.params.bookingId === ""
    ) {
  ...
  sendWebsocketMessage(id, method, params) {
    ...
    WebsocketClient.sendMessage(
      id || DEFAULT_QUOTE_LOC_ID,
      method || DEFAULT_QUOTE_LOC_METHOD,
      params
    );
  }
  ...
  stopQuote = preparedBookingId => {
    WebsocketClient.sendMessage(DEFAULT_QUOTE_LOC_ID, "approveQuote", {
      bookingId: preparedBookingId
    });
  };

  restartQuote = preparedBookingId => {
    WebsocketClient.sendMessage(DEFAULT_QUOTE_LOC_ID, "quoteLoc", {
      bookingId: preparedBookingId
    });
  };
      ...
      <View style={styles.floatingBar}>
        <View style={styles.detailsView}>
          <View style={styles.pricePeriodWrapper}>
            <Text style={[styles.price, styles.fontFuturaMed]}>
              {currencySign} {priceInCurrency.toFixed(2)}
            </Text>
            <Text style={[styles.period1, styles.fontFuturaStd]}>
              {" "}
              for {daysDifference} nights
            </Text>
          </View>
          <View style={styles.pricePeriodWrapper}>
            <Text style={[styles.price, styles.fontFuturaStd]}>
              {locAmount} LOC
            </Text>
            <Text style={[styles.period2, styles.fontFuturaStd]}>
              {" "}
              for {daysDifference} nights
            </Text>
          </View>
        ...
        <View style={styles.nextButtonView}>
          <TouchableOpacity
            style={isDisabled ? styles.nextButtonDisabled : styles.nextButton}
            onPress={onPressFunc}
          >
...
let mapStateToProps = (state, ownProps) => {
  ...
  let locAmount =
    locAmounts.locAmounts[DEFAULT_QUOTE_LOC_ID] &&
    locAmounts.locAmounts[DEFAULT_QUOTE_LOC_ID].locAmount &&
    locAmounts.locAmounts[DEFAULT_QUOTE_LOC_ID].locAmount.toFixed(2);
  const quoteLocError =
    locAmounts.locAmounts[DEFAULT_QUOTE_LOC_ID] &&
    locAmounts.locAmounts[DEFAULT_QUOTE_LOC_ID].error;
