...
class LocPrice extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (fromParentType === 0) {
      ...
      if (
        !this.isPause &&
        nextProps.nav.routes.length > 2 &&
        this.props.nav.routes.length === 2 &&
        this.props.nav.routes[1].routeName === "HotelsSearchScreen"
      ) {
        ...
        this.props.nav.routes.length > 2 &&
        nextProps.nav.routes.length === 2 &&
        nextProps.nav.routes[1].routeName === "HotelsSearchScreen"
      ) {
        this.resumeWebsocket(nextProps);
      }
    } else if (fromParentType === 1) {
      //available room
      if (
        !this.isPause &&
        nextProps.nav.routes.length > 3 &&
        this.props.nav.routes.length === 3 &&
        this.props.nav.routes[2].routeName === "HotelDetails"
      ) {
        this.pauseWebocket(nextProps);
      } else if (
        this.isPause &&
        this.props.nav.routes.length > 3 &&
        nextProps.nav.routes.length === 3 &&
        nextProps.nav.routes[2].routeName === "HotelDetails"
      ) {
        this.resumeWebsocket(nextProps);
      }
      ...
      if (
        ...
        nextProps.nav.routes.length > 2 &&
        this.props.nav.routes.length === 2 &&
        this.props.nav.routes[1].routeName === "HomesSearchScreen"
      ) {
        this.pauseWebocket(nextProps);
      } else if (
        this.isPause &&
        this.props.nav.routes.length > 2 &&
        nextProps.nav.routes.length === 2 &&
        nextProps.nav.routes[1].routeName === "HomesSearchScreen"
      ) {
        this.resumeWebsocket(nextProps);
      }
      ...
      if (
        !this.isPause &&
        nextProps.nav.routes.length > 4 &&
        this.props.nav.routes.length === 4 &&
        this.props.nav.routes[3].routeName === "HomeReviewScreen"
      ) {
        this.pauseWebocket(nextProps);
      } else if (
        this.isPause &&
        this.props.nav.routes.length > 4 &&
        nextProps.nav.routes.length === 4 &&
        nextProps.nav.routes[3].routeName === "HomeReviewScreen"
      ) {
        this.resumeWebsocket(nextProps);
      }
    } else {
      ...
      if (
        nextProps.isLocPriceWebsocketConnected &&
        nextProps.isLocPriceWebsocketConnected !==
          this.props.isLocPriceWebsocketConnected
      ) {
        WebsocketClient.sendMessage(this.state.fiatInEur, null, {
          fiatAmount: this.state.fiatInEur
        });
      }

      if (
        nextProps.exchangeRates.currencyExchangeRates &&
        !this.state.isLocPriceRendered
      ) {
        const fiatInEur =
          nextProps.exchangeRates.currencyExchangeRates &&
          CurrencyConverter.convert(
  ...
  render() {
    ...
    return (
      <Text style={this.props.style} numberOfLines={1} ellipsizeMode="tail">
        {hasBacket ? "(LOC " + locAmount + ")" : "LOC " + locAmount}
      </Text>
    );
  }
}
...
let mapStateToProps = (state, ownProps) => {
  ...
  let locAmount =
    locAmounts.locAmounts[fiatInEur] &&
    locAmounts.locAmounts[fiatInEur].locAmount.toFixed(2);
