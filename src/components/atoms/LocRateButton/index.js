...
class LocRateButton extends Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (isLocPriceWebsocketConnected && !this.isSendMessage) {
      ...
      WebsocketClient.sendMessage(exchangeRates.locRateFiatAmount, null, {
        fiatAmount: exchangeRates.locRateFiatAmount
      });
    }
  }
        ...
        <View style={styles.fab}>
          {locRate != 0 ? (
            <Text style={styles.fabText}>
              LOC/{currency} {parseFloat(locRate).toFixed(2)}
            </Text>
          ) : (
