...
class ProfileWalletCard extends Component {
    ...
    processWalletError(error) {
        this.props.setWalletData({walletState: WALLET_STATE.CONNECTION_ERROR, caller: 'ProfileWalletCard: case 8 - error'});
    }
    ...
    refreshWalletFromServer(callback = null) {
        ...
        if ((!isFirstLoading || skipLOCAddressRequest) &&  validateLOCAddress(locAddress) == 1) {
            this.props.setWalletData({walletState: WALLET_STATE.LOADING, caller: 'ProfileWalletCard: case 1 - loading (short)'});
            this.refreshWalletBalance();
            return;
        }

        this.props.setWalletData({walletState: WALLET_STATE.CHECKING, caller: 'ProfileWalletCard: case 2 - checking'});
                    ...
                    res.body
                        .then((data) => {
                            ...
                            switch (locAddressValidationResult) {
                                ...
                                case -1:
                                    this.props.setWalletData({locAddress, walletState: WALLET_STATE.NONE, caller: 'ProfileWalletCard: case 3 - none'});
                                    break;

                                case 0:
                                    this.props.setWalletData({locAddress, walletState: WALLET_STATE.INVALID, caller: 'ProfileWalletCard: case 4 - invalid'});
                                    break;

                                case 1:
                                    this.props.setWalletData({locAddress, walletState: WALLET_STATE.LOADING, caller: 'ProfileWalletCard: case 5 - loading'});
    ...
    refreshWalletBalance(changeState=false, locAddress=null) {
        if (changeState) this.props.setWalletData({walletState: WALLET_STATE.LOADING, caller: 'ProfileWalletCard: case 6 - loading'});
        ...
        if (addressValidationResult == 1) {
            ...
            console.warn('[ProfileWalletCard] Trying to get balance when wallet is not ready', {addressValidationResult, locAddress})
        }
    }
    ...
    createWallet() {
        ...
        const callback = (locAddressValidationResult) => {
            if (locAddressValidationResult != 1) {
                ...
                this.refs.toast.show("Your wallet was already created from another client.\nLoading it instead ...", 3000);
            }
        };
