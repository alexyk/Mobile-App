...
import { setLocPriceWebsocketConnection } from "../redux/action/exchangerSocket";
import { updateLocAmounts, clearLocAmounts } from "../redux/action/locAmounts";
import { setSeconds } from "../redux/action/locPriceUpdateTimer";
  ...
  handleRecieveMessage(event) {
    if (event) {
      ...
      if (!WS.self.grouping) {
        ...
        WS.self.locAmounts = [
          ...WS.self.locAmounts,
          { fiatAmount: data.id, params: data.params, error: data.error }
        ];
      }
    }
  }
