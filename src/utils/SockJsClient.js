...
class SockJsClient extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (this.state.connected) {
      ...
      Lo.difference(nextProps.topics, this.props.topics).forEach(newTopic => {
        this.log(`Subscribing to topic: ${newTopic}`);
        this.subscribe(newTopic);
      });

      // Unsubscribe from old topics
      Lo.difference(this.props.topics, nextProps.topics).forEach(oldTopic => {
  ...
  initStompClient = () => {
    ...
    this.client = Stomp.over(
      new SockJS(this.props.url, null, this.props.options)
    );
