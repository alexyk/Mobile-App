...
export default function createOfflineRequester() {
  ...
  const promiseRes = (data, title) => ({
    body: new Promise(function(success, reject) {
      try {
        ...
        const keysDetails = keys.map(
          key => `${key}:${isObject(data[key]) ? "{...}" : data[key]} `
        );
  ...
  const getOfflineResponse = function(title) {
    switch (title) {
      case "login":
        return require("./offline-responses/login.json");
      case "getUserInfo":
        return (value => {
          switch (value) {
            case -1:
              return require("./offline-responses/userInfo.json");
            case 0:
              return require("./offline-responses/userInfo-with-invalid-wallet.json");
            case 1:
              return require("./offline-responses/userInfo-with-wallet.json");
          }
        })(validationStateOfflineWallet);
      case "getCountries":
        return require("./offline-responses/countries.json");
      case "getCurrencyRates":
        return require("./offline-responses/rates.json");
      case "getLocRateByCurrency":
        return require("./offline-responses/convert.json");
      case "getRegionsBySearchParameter":
        return jsonRegions;
      case "getStaticHotels":
        return offlinePacksHotels[autoHotelSearchPlace].first;
      case "getMapInfo":
        return offlinePacksHotels[autoHotelSearchPlace].all;
      case "getHotelById":
        return require("./offline-responses/hotel1.json");
      case "getHotelRooms":
        return require("./offline-responses/rooms1.json");
      case "getConfigVarByName":
        return require("./offline-responses/payment-var.json");
      case "createReservation":
        return require("./offline-responses/booking1.json");
      case "getUserHasPendingBooking":
        return require("./offline-responses/booking1-pending.json");
      case "getListingsByFilter":
        return offlinePacksHomes[autoHomeSearchPlace];
      case "getMyHotelBookings":
        return require("./offline-responses/my-trips-all.json");
  ...
  const offlineRequester = {
    ...
    getRegionsBySearchParameter: (...args) =>
      genPromise(args, "getRegionsBySearchParameter"),
    ...
    getUserHasPendingBooking: (...args) =>
      genPromise(args, "getUserHasPendingBooking"),
    markQuoteIdAsMarked: (...args) =>
      genPromise(args, "getUserHasPendingBooking"),
    // homes
    getListingsByFilter: (...args) =>
      genPromise(args, "getListingsByFilter", 1),
    ...
    startSocketConnection: (onData, _this) => {
      ...
      try {
        ...
        processError(
          `[offline::startSocketConnection] Error in getting offline pack: ${e.message}`,
          { error: e }
        );
      }
...
const locAmount = {
  fiatAmount: "1000",
  params: { locAmount: 1323.7014764150367 },
  error: undefined
};
